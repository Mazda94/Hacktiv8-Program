...
function questLogger(quests, actions) {
    ...
    for (let i = 0; i < quests.length; i++) {
        let obj = {}
        obj.quests = quests[i]['name']

        let temp = 0
        for (let j = 0; j < quests[i]['requirements'].length; j++) {
        ...
        if (temp === quests[i]['requirements'].length) {
...
console.log('====================')
    // [
    //   { name: 'berkuda', status: 'failed'
    //   { name: 'kaya', status: 'failed'
    //   { name: 'minum kopi', status: 'failed'
    //   { name: 'menikmati senja', status: 'failed'`
    //  ]